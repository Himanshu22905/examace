  ...
  return (
    <div style={{ padding:"20px 16px", maxWidth:1000 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }} className="grid-4">
        {[["📝","Tests",totalTests,"#38BDF8"],["⭐","Avg Score",avgScore,"#E8B84B"],["🎯","Accuracy",avgAccuracy+"%","#34D399"],["🏆","Best",bestScore,"#A78BFA"]].map(([icon,label,val,color],i)=>(
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:16 }} className="grid-2">
        <div className="card">
          ...
          {attempts.length===0 ? (
            <div style={{ textAlign:"center", padding:"20px 0", color:"#7090B0" }}><div style={{ fontSize:28, marginBottom:8 }}>📊</div><div style={{ fontSize:12 }}>No tests yet</div></div>
...
export default function Dashboard() {
  ...
  const content = () => {
    switch(page) {
      case "home":    return <HomePage profile={profile} attempts={attempts} tests={tests} firstName={firstName} setPage={setPage}/>;
      case "tests":   return <TestsPage tests={tests}/>;
      case "history": return <HistoryPage attempts={attempts}/>;
      case "profile": return <ProfilePage user={user} profile={profile} attempts={attempts} onLogout={handleLogout} onProfileSaved={handleProfileSaved}/>;
      default:        return <HomePage profile={profile} attempts={attempts} tests={tests} firstName={firstName} setPage={setPage}/>;
    }
  };
